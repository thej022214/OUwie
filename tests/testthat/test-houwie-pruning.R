...
test_that("hOUwie runs on the pruning path and legacy selects the sampler", {
  skip_on_cran()
  fixture <- pruning_fixture(seed = 96011, n_tip = 12)
  options <- list(algorithm = "NLOPT_LN_SBPLX", maxeval = 20,
                  ftol_rel = .Machine$double.eps^0.25)
  fit <- hOUwie(fixture$phy, fixture$data, rate.cat = 1,
                discrete_model = "ER", continuous_model = "OUM", nSim = 10,
                algorithm = "pruning", resolution = 1L, max_components = 4,
                opts = options, quiet = TRUE)
  expect_true(is.finite(fit$loglik))
  expect_identical(fit$algorithm, "pruning")

  legacy_fit <- hOUwie(fixture$phy, fixture$data, rate.cat = 1,
                       discrete_model = "ER", continuous_model = "OUM",
                       nSim = 10, legacy = TRUE, opts = options, quiet = TRUE)
  expect_identical(legacy_fit$algorithm, "sampling")
})

test_that("the component ceiling defaults on tip ambiguity, not regime count", {
  skip_on_cran()
  expect_identical(getDefaultMaxComponents(list(1L, 2L, 1L)), 16L)
  expect_identical(getDefaultMaxComponents(list(1L, c(1L, 2L), 2L)), 32L)

  # hidden states make every tip ambiguous between its rate categories, which is
  # the case that needs the larger ceiling
  fixture <- pruning_fixture(seed = 96031, n_tip = 12)
  slice <- max(branching.times(fixture$phy)) + 1
  for(rate.cat in 1:2){
    n_states <- 2 * rate.cat
    edge_liks <- getEdgeLiks(fixture$phy, fixture$data[, 1:2], n_states,
                             rate.cat, slice)
    tip_sets <- getTipStateSets(fixture$phy, edge_liks, n_states)
    expect_identical(getDefaultMaxComponents(tip_sets),
                     if(rate.cat == 1L) 16L else 32L)
  }
})

test_that("reconstruction is refused on the pruning path", {
  skip_on_cran()
  fixture <- pruning_fixture(seed = 96021, n_tip = 8)
  expect_error(
    hOUwie(fixture$phy, fixture$data, rate.cat = 1, discrete_model = "ER",
           continuous_model = "OUM", nSim = 10, algorithm = "pruning",
           recon = TRUE, quiet = TRUE),
    "recon is not available"
  )
})
