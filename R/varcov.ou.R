...
varcov.ou <- function(phy, edges, Rate.mat, root.state, simmap.tree = FALSE, root.age = NULL, scaleHeight = FALSE, assume.station = TRUE, shift.point = 0.5) {
	...
	if(simmap.tree == TRUE){
		for(i in 1:nrow(edges)){
			...
			Ato[desc] <- Acur
			n.cov1[desc,] <- nodevar1[i]
			n.cov2[desc,] <- nodevar2[i]
		}
	}else{
		for(i in 1:nrow(edges)){
			anc <- edges[i,2]
			...
			oldtime <- edges[i,4]
			newtime <- edges[i,5]
			#figure out regimes for this edge
			if(anc %in% edges[,3]){
			...
			if(oldregime == newregime){
				...
				s2 <- sigma[newregime]
				nodevar1[i] <- a * dt
				nodevar2[i] <- s2 * exp(2 * Acur) * (exp(2 * a * dt) - 1) / (2 * a)
				Acur <- Acur + a * dt
			}else{
				...
				a1 <- alpha[oldregime]
				s21 <- sigma[oldregime]
				Q1 <- s21 * exp(2 * Acur) * (exp(2 * a1 * dt1) - 1) / (2 * a1)
				Acur <- Acur + a1 * dt1
				#epoch 2 - new regime
				dt2 <- newtime - shifttime
				a2  <- alpha[newregime]
				s22 <- sigma[newregime]
				Q2 <- s22 * exp(2 * Acur) * (exp(2 * a2 * dt2) - 1) / (2 * a2)
				Acur <- Acur + a2 * dt2
				nodevar1[i] <- a1 * dt1 + a2 * dt2
				nodevar2[i] <- Q1 + Q2
			}
			Ato[desc] <- Acur
			n.cov1[desc,] <- nodevar1[i]
...
# Bad version that produces incorrect VCV when alphas differ:
varcov.ou.old <- function(phy, edges, Rate.mat, root.state, simmap.tree=FALSE, root.age=NULL, scaleHeight=FALSE, assume.station=TRUE, shift.point=.5){
